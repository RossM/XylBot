...
sub cmd_start {
	...
	announce "Type \"!$mafia_cmd go\" to immediately end signups.";
...
sub cmd_in {
	...
	elsif ($fromnick =~ /^none$|^nolynch$|^self$|^sky$|^town$|^mafia$|^sk$|^unknown$/i)
	{
...
sub mafia_command {
	...
	{
		if ($modactionsallowed) && $level >= 100 && $to eq $mafiachannel
		{
			cancel_game();
		}
		else
	...
	elsif ($subcommand eq 'debugcheckstatus')
	{
		...
		eval {
			...
			foreach my $result (@results)
			{
				...
				{
					$result = join(' ', map { "$_=$result->{$_}" } sort keys %$result);
				}
			}
	...
	{
		announce($to, "Mafia commands: " . join(' ', sort qw[start go wait in out votes alive special help reset stop status showcommands testsetup coin top10 record leaders checkmod rules unmute]));
	}
	elsif ($subcommand eq 'showmodcommands' && $forum eq 'public')
	{
		announce($to, "Moderator commands: " . join(' ', sort qw[settest starttest forcein forceout forcevote forceaction forcehelp force forcenextphase forcerole modkill reset stop debugcheckstatus rolescript showsetup usepresetup showpresetup baserole addability removeability addstatus removestatus setbuddy setdesc begin]));
	}
...
#			($role_config{$a->{role}} && $role_config{$a->{role}}{name} =~ /^Townie$|^Mafioso$/) <=> ($role_config{$b->{role}} && $role_config{$b->{role}}{name} =~ /^Townie$|^Mafioso$/) ||
	...
	elsif ($subcommand eq 'phayt')
	{
		notice ($fromnick, "Phayt is travelling through the United States. If you want him to visit you, email him at phay7e(at)gmail.com or phone (205)588-9229");
	}
	...
	if ($@)
	{
		...
		my $err = $@;
		$err =~ s/\n$//;
		notice($fromnick, "Oops! Due to a bug, your command failed. Please report this problem to " . $::owner .  ".");
...
sub help {
	...
	notice($player, "To see a list of commands, type \"!$mafia_cmd showcommands\". For help on playing the game, type \"/msg $::nick help basics\".");
...
sub action {
	...
	foreach my $longaction (@actions, @group_actions, "none")
	{
		...
		if (lc $command eq lc $action)
		{
			...
			if ($player_data{$player}{phase_action} && !$free)
			{
			...
			$player_data{$player}{phase_action} = $longaction unless $free;
			$player_data{$player}{cur_targets} = [@targets] unless $free;
...
players to choose their own roles. To get a list of all possible setups type "!$mafia_cmd special". For
...
HELP
	::add_help "$mafia_cmd go", "$mafia_cmd go: Immediately begins a game of mafia without waiting for more players.";
...
HELP
	...
	::add_help "$mafia_cmd votes", "$mafia_cmd votes: Displays the current vote count. If nothing happens, wait a few seconds and try again.";
	...
	::add_help "$mafia_cmd reset", "$mafia_cmd reset: Votes to reset the bot. If enough people vote for a reset, the game will be stopped and the bot reset.";
	...
	::add_help "$mafia_cmd testsetup", "$mafia_cmd testsetup [setup] [players]: Gives an example setup.";
	...
	::add_help "$mafia_cmd resolvemode", "$mafia_cmd resolvemode <classic|paradox>: Sets the action resolution mode, which determines the order of night actions. Classic mode processes actions in a fixed order, while paradox mode sorts them in a hopefully intuitive fashion. This is a privileged command.";
	...
	::add_help "$mafia_cmd changesetup", "$mafia_cmd changesetup [setup]: Changes the setup of a game during the signup phase. This is a privileged command.";
	...
	::add_help "$mafia_cmd forcenextphase", "$mafia_cmd forcenextphase: Causes the game to move to the next phase immediately. This is a moderator-only command.";
	...
	::add_help "$mafia_cmd forcehelp", "$mafia_cmd forcehelp <player>: Resends a player's role PM to that player. This is a moderator-only command.";
	::add_help "$mafia_cmd debugcheckstatus", "$mafia_cmd debugcheckstatus <player>: Displays the current status effects on a player. moderator-only command.";
	::add_help "$mafia_cmd showsetup", "$mafia_cmd showsetup: Shows the role PMs of all players. This is a moderator-only command.";
	::add_help "$mafia_cmd forcerole", "$mafia_cmd forcerole <player> <role>: Forces a player into a specific role name. This is a moderator-only command.";
	...
	::add_help "$mafia_cmd modkill", "$mafia_cmd modkill [player]: Immediately kills a player. This is a moderator-only command.";
...
You can also vote to end the day without lynching with "!vote no lynch". No lynch happens when enough
...
"!$mafia_cmd votes".
HELP
	::add_command_public "unvote", \&unvote, "unvote: Removes your current vote.";
	::add_command_public "take", \&command_take, "take [item]: Takes an item.";
	::add_command_public "drop", \&command_drop, "drop [item]: Drops an item.";
	::add_command_public "buy", \&command_buy, "buy [item]: Buys an item.";
...
First of all, type "!$mafia_cmd rules" to see the channel rules. 
-
To start a game, type "!$mafia_cmd start" in the channel. If you prefer, you can start a specific setup
such as 'straight' or 'mild' by using "!$mafia_cmd start [setup]". To get a list of setups, use
"!$mafia_cmd special". Beginners should stick to the first few setups on the list.
-
Once a game has been started, join it by typing "!$mafia_cmd in". You have to do this even if you're the
person who started the game. Signups end automatically after a short time. If all interested players
...
revoting. For more details, type "/msg $::nick help vote".
BASICS
...
don't want to use, type "/msg $::nick none" so that the bot knows not to wait for you. If you're
mafia, only one member of the mafia can use the mafiakill action, so discuss who will make the kill
with your teammates.
-
Actions that start with "(day)" are day actions; leave off the "(day)" when you submit them. Actions
that start with "(x)" can be used during either day or night. A few roles have actions marked "(auto)",
which means you will use them automatically every night. They don't count as your action for the night.
ACTIONS
	::add_help "rules", <<RULES;
Failing to follow these rules may result in being modkilled or banned by mafia moderators/operators.
The ops may also use their own discretion in managing the channel. 
If you have any questions, concerns or suggestions, please do not hesitate to pm Daz on IRC.

1. All messages that you submit must be viewable by all players unless you are part of the same scumteam/mason group
2. You may not directly quote any messages you receive from the bot privately. You may paraphrase if you want.
3. You may not impersonate a missing player unless you are replacing them into the current game.
4. You must attempt to fulfill your win condition in all games.
5. You may not bring outside information or previous vendettas into the current game. This does not include meta-plays or bot knowledge.
6. Do not join games you cannot complete. If you need to leave, say so and '/nick YourNick[AFK]' before you go.
7. Do not abuse server/bot lag or bugged bot mechanics.
8. If you are using an excess of bot commands, please do so in a query so as not to flood the channel. This includes mass-help and testsetuping.
9. Do not spam the channel - use http://xylbot.pastebin.com/ for long pastes
10. Undetectable fakevotes must not be used - in particular alt or colour codes. If in doubt, don't use.
RULES
